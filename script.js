document.addEventListener('DOMContentLoaded', function() {
    ...
    // Reset and initialize visitor counter
    resetVisitorCounter();
    initVisitorCounter();
    ...
    // Reset visitor counter - clears all visitor data and sets initial count
    function resetVisitorCounter() {
        // Set initial count to a number between 1 and 10 (for a fresh start)
        localStorage.setItem('totalVisits', '1');
        // Clear all visitor IDs
        localStorage.removeItem('visitedUsers');
        // Keep the current visitor ID or create a new one if none exists
        getOrCreateVisitorId();
        
        console.log('Visitor counter has been reset.');
    }
    ...
    function initVisitorCounter() {
        ...
        const visitedUsers = getVisitedUsers();
        
        // If no total visits stored yet, initialize with 1
        if (!totalVisits) {
            totalVisits = 1;
        }
        
        // Check if this user has already been counted
        const isNewVisitor = !visitedUsers.includes(visitorId);
        
        // Only increment the counter for new visitors
        if (isNewVisitor) {
            totalVisits = parseInt(totalVisits) + 1;
            
            // Add this user to the visited users list
            visitedUsers.push(visitorId);
        ...
        if (visitorCountDisplay) {
            // Show a special indicator for new visitors
            if (isNewVisitor) {
                visitorCountDisplay.dataset.newVisitor = "true";
                setTimeout(() => {
                    visitorCountDisplay.dataset.newVisitor = "false";
                }, 3000);
            }
            
            animateCounter(0, parseInt(totalVisits), visitorCountDisplay);
        }
    }
    ...
    // Get array of visitors who have already been counted
    function getVisitedUsers() {
        const visitedUsers = localStorage.getItem('visitedUsers');
        return visitedUsers ? JSON.parse(visitedUsers) : [];
    }
